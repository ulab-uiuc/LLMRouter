...
def train_from_config(config: dict):
    ...
    encoder_model = DebertaV2Model.from_pretrained(model_cfg['backbone'])
    ...
    model = RouterModule(
        ...
        hidden_state_dim=model_cfg['hidden_state_dim'],
        node_size=num_llms,
    ...
    print("=" * sep_width)
    print(f"Checkpoints saved to: {output_cfg['save_path']}")
...
def main():
    ...
    parser.add_argument(
        '--config',
        type=str,
        default="llmrouter/configs/routerdc_nq.yaml",
