...
class SmallestLLM(MetaRouter):
    """
    Always select the smallest LLM (based on the 'size' field) that ends with 'B'.
    """
    ...
    def inference(self):
        ...
        # Find the smallest model among those
        ...
        print(f"🐜 Smallest model (ending with 'B') selected: {smallest_model_name} ({smallest_model.get('size')})")
